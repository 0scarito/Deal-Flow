...
function renderDeals(){
  ...
  data.forEach(d=>{
    ...
    r.innerHTML='<td style="text-align:center;"><input type="checkbox" class="rowSel" data-key="'+escH(k)+'"'+checked+' onclick="event.stopPropagation();onDealRowSel(this)"/></td><td class="mono">'+escH(d.date)+'</td><td>'+av+'</td><td style="font-weight:500;white-space:nowrap;">'+escH(d.client)+'</td><td style="color:var(--text2);font-size:11px;">'+escH(d.contrat)+'</td><td>'+escH(d.produit)+'</td><td style="color:var(--text2);font-size:11px;">'+(d.produit_type?escH(d.produit_type):'—')+'</td><td>'+escH(d.fourn)+'</td><td style="color:var(--text2);">'+(d.broker?escH(d.broker):'—')+'</td><td style="text-align:right;" class="mono">'+f0(d.nom)+'</td><td>'+escH(d.dev)+'</td><td class="mono" style="font-size:10px;color:var(--text2);">'+(d.isin?escH(d.isin):'—')+'</td><td class="mono" style="font-size:11px;color:var(--text2);">'+(d.issue?escH(d.issue):'—')+'</td><td class="mono" style="font-size:11px;color:var(--text2);">'+(d.terme?escH(d.terme):'—')+'</td><td>'+tBadge(d.ct)+'</td><td style="text-align:right;color:var(--blue);font-weight:500;">'+(d.ufE>0?fE(d.ufE):'—')+'</td><td style="text-align:right;color:var(--green);font-weight:500;">'+(d.runE>0?fE(d.runE):'—')+'</td><td class="mono" style="font-size:11px;">'+(d.fRef?escH(d.fRef):'—')+'</td><td>'+fBadge(d.fSt)+'</td><td style="font-size:11px;color:var(--text2);max-width:120px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">'+(d.notes?escH(d.notes):'—')+'</td><td class="mono" style="font-size:11px;color:var(--text2);">'+(d.invS?escH(d.invS):'—')+'</td><td class="mono" style="font-size:11px;color:var(--text2);">'+(d.inv?escH(d.inv):'—')+'</td><td><div style="display:flex;gap:5px;justify-content:flex-end;align-items:center;"><button class="btn btn-sm" onclick="event.stopPropagation();openDealModal('+deals.indexOf(d)+')">Modifier</button><button class="btn btn-sm" style="color:var(--red);border-color:var(--red-bg);" onclick="event.stopPropagation();deleteDeal('+deals.indexOf(d)+')">Supprimer</button></div></td>';
  });
